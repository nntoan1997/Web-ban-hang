...
if(isset($_POST["act"]))
{
	...
	$noibat= $_POST['noibat'];
	$splitmt= split("\,",$mota);
	for($i= 0;$i < count($splitmt);$i++){
		$mt.= "<p>".$splitmt[$i]."</p>";
	}
	$mota_ngan = $_POST['mota_ngan'];
	$splitmtn= split("\,",$mota_ngan);
	for($i= 0;$i < count($splitmtn);$i++){
		$mtn.= "<p>".$splitmtn[$i]."</p>";
	}
...
//	echo "$name - $new<hr>";
	...
	{	
		if($id2!=$id){
			$sql5="UPDATE sanpham SET id='$id2', id_loai='$loaisp', tensp='$tensp', mota='$mt', gia='$gia',ghichu='$ghichu',hinh='$new',id_menu='$menu',mota_ngan='$mtn',noibat='$noibat' WHERE id='$id'";
			...
			$sql3="UPDATE sanpham SET id='$id2', id_loai='$loaisp', tensp='$tensp', mota='$mt', gia='$gia',ghichu='$ghichu',id_menu='$menu', mota_ngan='$mtn',noibat='$noibat' WHERE id='$id'";
		...
		unlink("../sanpham/large/$name");
			...
			//********************************resize hinh ********************************
		$sql4="UPDATE sanpham SET id='$id2', id_loai='$loaisp', tensp='$tensp', mota='$mt',hinh='$newName', gia='$gia',ghichu='$ghichu',id_menu='$menu',mota_ngan='$mtn',noibat='$noibat' WHERE id='$id'";
...
<?php
	...
	else 
	{
		...
		$mota_ngan=$r["mota_ngan"];$noibat=$r["noibat"];
	//	echo "$id_loai";
?>
<table width="740" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr bgcolor="#FFFFFF">
    <td style="padding-left:80px" height="30">Mô tả ngắn:</td>   
     <td><textarea name="mota_ngan" cols="27" rows="5"  style="width:240px"><?php echo "$mota_ngan" ?></textarea> </td>
  </tr>
...
<img align="middle" src="/sanpham/small/<?php echo "$hinh"; ?>" width="80" height="80">
 <input type="hidden" name="oldimage" value="<?php echo "$hinh"; ?>"> 
  ...
  <tr>
    <td style="padding-left:80px" height="30">Sản phẩm nổi bật:</td>
    <td>
    <select name="ghichu" style="width:240px">
	<?php echo Getnoibat($id); ?>
    </select>
    </td>
  </tr>  
  <tr>
