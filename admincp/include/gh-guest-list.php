...
<form method="post">
  ...
  <?php
	...
	//******************************** Nội Dung *********************************//  
  	...
  	$sql="select hoadon.ngaydat, hoadon.hoten, sanpham.tensp, hoadon.soluong, hoadon.tongtien from hoadon,sanpham where hoadon.id=sanpham.id and hoadon.tinhtrang='dathang' order by ngaydat DESC ";
	//echo "$sql";
	$kq=mysql_query($sql);
	if(!$kq)
		echo "";
	...
	while($r=mysql_fetch_array($kq))
	{
		...
		$hoten=$r["hoten"];$tensp=$r["tensp"];$gia=$r["tongtien"];$gia2=number_format($gia,0,'','.');
